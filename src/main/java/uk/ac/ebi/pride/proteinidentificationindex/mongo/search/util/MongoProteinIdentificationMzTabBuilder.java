...
public class MongoProteinIdentificationMzTabBuilder {
  ...
  private static Logger logger = LoggerFactory.getLogger(MongoProteinIdentificationMzTabBuilder.class.getName());

  public static String OPTIONAL_SEQUENCE_COLUMN   = "protein_sequence";

  public static List<MongoProteinIdentification> readProteinIdentificationsFromMzTabFile(String projectAccession, String assayAccession, MZTabFile tabFile) {
    ...
    if (tabFile != null) {
      ...
      for (Protein mzTabProtein: mzTabProteins) {
        ...
        mongoProteinIdentification.setId(MongoProteinIdentificationIdBuilder.getId(mzTabProtein.getAccession(), projectAccession, assayAccession));
        ...
        mongoProteinIdentification.setAmbiguityGroupSubmittedAccessions(new LinkedList<String>());
        ...
        if (mzTabProtein.getAmbiguityMembers()!=null && mzTabProtein.getAmbiguityMembers().size()>0 && !mzTabProtein.getAmbiguityMembers().get(0).equals("null")) {
          mongoProteinIdentification.getAmbiguityGroupSubmittedAccessions().addAll(mzTabProtein.getAmbiguityMembers());
        }
        mongoProteinIdentification.setModifications(new LinkedList<ModificationProvider>());
        if (mzTabProtein.getModifications()!=null && mzTabProtein.getModifications().size()>0 && !mzTabProtein.getModifications().get(0).equals("null")) {
          for (Modification mod: mzTabProtein.getModifications())
            mongoProteinIdentification.addModification(ModificationHelper.convertToModificationProvider(mod));
        }
        try {
